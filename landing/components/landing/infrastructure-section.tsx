...
const regions = [
  { name: "AAVE V3", nodes: "lending", status: "live" },
  { name: "Benqi", nodes: "lending", status: "live" },
  { name: "MockPool A", nodes: "yield", status: "live" },
  { name: "MockPool B", nodes: "yield", status: "live" },
];
          ...
          <div className="grid lg:grid-cols-[auto_1fr] gap-8 lg:gap-16 items-stretch">
            ...
            <div className="flex flex-col justify-center">
              ...
              <p className={`mt-8 text-xl text-muted-foreground leading-relaxed max-w-lg transition-all duration-1000 delay-100 ${
                ...
                Every deposit, job, and rebalance settles on Avalanche.
                Funds stay non-custodial and always verifiable.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`lg:col-span-2 relative p-8 lg:p-12 border border-foreground/10 bg-foreground/[0.02] overflow-hidden transition-all duration-700 ${
            ...
            <div className="relative z-10">
              <div className="flex items-baseline gap-2 mb-4">
                ...
                <span className="text-2xl text-muted-foreground">protocols</span>
              </div>
              <p className="text-muted-foreground max-w-md">
                Yield sources integrated as adapters and continuously compared, so capital always flows to the best net APY.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-6">
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-100 ${
              ...
              <span className="block text-sm text-muted-foreground mt-2">Funds always earning</span>
            </div>
            
            <div className={`p-8 border border-foreground/10 bg-foreground/[0.02] transition-all duration-700 delay-200 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              <span className="text-5xl lg:text-6xl font-display">Atomic</span>
              <span className="block text-sm text-muted-foreground mt-2">Rebalance &amp; settle</span>
            </div>
          </div>
        </div>
        ...
        <div className={`mt-12 grid grid-cols-2 lg:grid-cols-4 gap-4 transition-all duration-1000 delay-300 ${
          ...
          {regions.map((region, index) => (
            ...
            >
              ...
              <span className="text-sm text-muted-foreground">{region.nodes} adapter</span>
            </div>
          ))}
        </div>
      </div>
