...
/// @title MockPoolA
/// @notice Demo yield pool with a settable APY that ACTUALLY accrues yield over
///         time (like a real lending market), so the vault's share value grows
///         while parked here. Owner sets APY and pre-funds a USDC yield reserve;
///         getBalance() then grows block-by-block at that APY, drawing from the
///         reserve. Real APY math, no time acceleration.
contract MockPoolA is IYieldAdapter {
    ...
    /// @param initialAPY  Starting APY in basis points (e.g., 500 = 5.00%)
    constructor(address _vault, address _usdc, uint256 initialAPY) {
        require(_vault != address(0), "MockPoolA: zero vault");
        require(_usdc  != address(0), "MockPoolA: zero usdc");
        vault  = _vault;
        owner  = msg.sender;
        usdc   = IERC20(_usdc);
        _apy   = initialAPY;
    ...
    modifier onlyVault() { require(msg.sender == vault, "MockPoolA: only vault"); _; }
    modifier onlyOwner() { require(msg.sender == owner, "MockPoolA: only owner"); _; }

    // ─── Yield accrual ─────────────────────────────────────────────────────────

    /// @dev Yield earned but not yet realized, computed at the current APY since
    ///      the last accrual. Capped by the funded reserve so the pool stays
    ///      solvent (it can never owe more USDC than it actually holds).
    ...
    /// @dev Realize pending yield into the balance, drawing from the reserve.
    ...
    function fundYieldReserve(uint256 amount) external onlyOwner {
        _accrue();
        require(usdc.transferFrom(msg.sender, address(this), amount), "MockPoolA: reserve transfer failed");
        _reserve += amount;
        emit YieldReserveFunded(amount, _reserve);
    }

    /// @notice Set APY for demo purposes. Only callable by deployer.
    /// @param  newAPY New APY in basis points (e.g., 1800 = 18.00%)
    function setAPY(uint256 newAPY) external onlyOwner {
        require(newAPY <= 50_000, "MockPoolA: APY capped at 500%");
        _accrue(); // settle the prior period at the old rate before switching
        emit APYUpdated(_apy, newAPY, msg.sender);
        _apy = newAPY;
    }

    /// @notice Instantly credit simulated yield (a lump sum, not time-based).
    ///         Owner-only; caller must approve this contract for `amount` first.
    function creditYield(uint256 amount) external onlyOwner {
        _accrue();
        require(usdc.transferFrom(msg.sender, address(this), amount), "MockPoolA: yield transfer failed");
        _balance += amount;
    }
    ...
    function withdraw(uint256 amount) external override onlyVault {
        _accrue();
        require(_balance >= amount, "MockPoolA: insufficient balance");
        _balance -= amount;
    ...
    /// @inheritdoc IYieldAdapter
    /// @dev Live value owed to the vault: realized balance plus yield accrued
    ///      since the last interaction. Grows every block.
    ...
    /// @inheritdoc IYieldAdapter
    function protocolName() external pure override returns (string memory) { return "MockPoolA"; }
}
